"""
The MIT License (MIT)
Copyright © 2018 Jean-Christophe Bos & HC² (static.hc2.fr)

Original version from https://github.com/jczic/MicroWebSrv
Changes: optimized for lightweight non-blocking operation
"""
...
from lib.MicroWebSrv.microWebTemplate import MicroWebTemplate
...
class MicroWebSrv:
    ...
    def run_once(self):
        """Poll sockets once, non-blocking"""
        try:
            readable, _, _ = select.select([self._server], (), (), 0)
            if self._server in readable:
                client, cliAddr = self._server.accept()

                client_obj = self._client(self, client, cliAddr)
                if client_obj.ws is not None:
                    self.open_websockets.append(client_obj.ws)
        ...
        closed_sockets = []
        for ws in self.open_websockets:
            if not ws.process_ws():
                closed_sockets.append(ws)

        for ws in closed_sockets:
            self.open_websockets.remove(ws)

        return True

    def broadcast_on_ws(self, msg: str):
        # _, writable, _ = select.select((), self.open_websockets, (), 0)
            ...
            try:
                ...
                if self._parseFirstLine(response):
                    if self._parseHeader(response):
                        ...
                        elif 'MicroWebSocket' in globals() \
                                and self._microWebSrv.AcceptWebSocketCallback:
                            print("Creating WS")
                            self.ws = MicroWebSocket(socket=self._socket,
                                                     httpClient=self,
