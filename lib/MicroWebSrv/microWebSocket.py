...
class MicroWebSocket:
    ...
    def init_ws(self, accept_callback):
        self._socket.settimeout(3600)
        self._closed = False
        try:
            accept_callback(self, self._httpCli)
        except Exception as ex:
            print("MicroWebSocket : Error on accept callback (%s)." % str(ex))

    def process_ws(self):
        if not self._try_receive_frame():
            self.Close()
    ...
    def _try_receive_frame(self):
        try:
            readable, _, _ = select.select([self._socketfile], (), (), 0)
        except OSError:
            return True

        if self._socketfile not in readable:
            return True
